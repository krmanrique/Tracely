...
export default function GradePanel({ course, onClose }) {
  // Normalizar cortes — acepta tanto formato mock como formato backend
  const cortes = (course.cortes ?? []).map((ct) => ({
    id:     ct.id,
  ...
  const [grades, setGrades] = useState(
    Object.fromEntries(
      students.map((s) => [
        s.id,
        Object.fromEntries(cortes.map((c) => [c.id, [...(course.grades?.[s.id]?.[c.id] ?? [])]])),
      ])
    )
  );
  ...
  const activities = grades[students[0]?.id]?.[activeCorteId] ?? [];
  ...
  const addActivity = () => {
    ...
    setGrades((prev) => {
      ...
      students.forEach((s) => {
        next[s.id] = {
          ...next[s.id],
          [activeCorteId]: [...(next[s.id]?.[activeCorteId] ?? []), { id, label: newLabel.trim(), tipo: newTipo, value: null }],
        };
      });
      return next;
    });
  ...
  const removeActivity = (actId) => {
    setGrades((prev) => {
      const next = { ...prev };
      students.forEach((s) => {
        next[s.id] = { ...next[s.id], [activeCorteId]: (next[s.id]?.[activeCorteId] ?? []).filter((a) => a.id !== actId) };
      });
      return next;
    });
  };
  ...
  const updateGrade = (sid, actId, val) => {
    ...
    setGrades((prev) => ({
      ...
      [sid]: {
        ...prev[sid],
        [activeCorteId]: (prev[sid]?.[activeCorteId] ?? []).map((a) =>
  ...
  const studentAvg = (sid) => {
    const acts = grades[sid]?.[activeCorteId] ?? [];
  ...
  if (!cortes.length) return (
    <div className="overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="panel">
        ...
        <div className="empty"><div className="empty-icon">📝</div>No hay cortes configurados para este curso</div>
      </div>
    </div>
  );
    ...
    <div className="overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="panel">
        ...
        <div style={{ background: 'var(--bg3)', borderRadius: 11, padding: '12px 16px', marginBottom: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <input className="input-field" style={{ flex: 2, minWidth: 150 }}
              placeholder="Nombre..." value={newLabel}
        ...
        {students.length === 0 ? (
          ...
          <div className="empty"><div className="empty-icon">📝</div>Agrega una actividad para ingresar notas</div>
        ) : (
          <>
            {/* Encabezado tabla */}
              ...
              {activities.map((act) => (
                <div key={act.id} style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                    ...
                    <button onClick={() => removeActivity(act.id)} style={{ background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: 11 }}>✕</button>
                  </div>
                </div>
              ))}
            ...
            {students.map((s) => {
              ...
              const sg  = grades[s.id]?.[activeCorteId] ?? [];
              return (
                <div key={s.id} style={{ display: 'grid', gridTemplateColumns: `180px repeat(${activities.length}, 1fr) 70px`, gap: 6, padding: '8px 0', borderBottom: '1px solid var(--border)', alignItems: 'center' }}>
