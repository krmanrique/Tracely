...
export default function StudentDashboard({ estudianteId, semestre, semData, onNavigate, onNotifClick }) {
  ...
  if (loading) return (
    <div className="empty"><div className="empty-icon">⏳</div>Cargando dashboard...</div>
  );
  if (error) return (
    <div className="empty"><div className="empty-icon">⚠️</div>{error}</div>
  );
  if (!data) return (
    <div className="empty"><div className="empty-icon">📅</div>No hay datos para este semestre</div>
  );
    ...
    <div>
      ...
      <div className="grid grid-3" style={{ marginBottom: 18 }}>
        ...
        <div className="card" style={{ padding: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 20, flexDirection: 'column' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <span className={`badge badge-${riskLevel === 'low' ? 'active' : riskLevel === 'medium' ? 'warning' : 'critical'}`} style={{ fontSize: 12, padding: '4px 12px' }}>
      ...
      <div className="card" style={{ padding: 22 }}>
        <div className="section-header">
          ...
          <button className="section-link" onClick={() => onNavigate && onNavigate('grades')}>Ver notas detalladas →</button>
        </div>
        {courses.length === 0 ? (
